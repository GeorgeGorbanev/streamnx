...
func TestCallback_ConvertTrackSpotifyToYandex(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:        "convert_track/spotify/7uv632EkfwYhXoqf8rhYrg/yandex",
		...
		},
		{
			...
			input:        "convert_track/spotify/7DSAEUvxU8FajXtRloy8M0/yandex",
			expectedText: "Track not found in Yandex",
			fixturesMap: utils.FixturesMap{
				SpotifyTracks: map[string][]byte{
		...
		},
		{
			...
			input:        "convert_track/spotify/3NHSz1GyC5IeK1soZSjIIX/yandex",
		...
		},
		{
			...
			input:        "convert_track/spotify/2sP5VgY8PWb6c9DhgZEpSv/yandex",
