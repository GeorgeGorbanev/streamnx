...
func TestYandexCatalogFetchTrack(t *testing.T) {
	...
	got, err := catalog.FetchTrack(t.Context(), streamnx.Yandex, yandexTrackKey)
	...
	require.Equal(t, streamnx.Track{
		...
		URL:      "https://music.yandex.com/album/" + yandexAlbumID + "/track/" + yandexTrackID,
	}, got)
}
...
func TestYandexCatalogFetchTrackNotFound(t *testing.T) {
	...
	catalog := newYandexCatalog(t, server.URL)
	got, err := catalog.FetchTrack(
		t.Context(),
		streamnx.Yandex,
		yandexAlbumID+":"+yandexMissingTrackID,
	)
